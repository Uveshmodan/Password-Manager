...
@Composable
fun InsertPasswordSheet(
    onDismiss: () -> Unit, // Callback function when the sheet is dismissed
    onInsert: (PasswordTable) -> Unit // Callback function when inserting a password
) {
        ...
        Column(
            ...
            Button(
                onClick = {
                    ...
                    if (error.isEmpty()) {
                        val data =
                            PasswordTable(accountName = name, emailUname = email, password = password)
                        onInsert(data) // Call the insert callback with the password data
